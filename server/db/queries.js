...
export async function getTopProducts(count) {
    const result = await pool.query("SELECT p.id , p.product_name, p.product_image_url as url, EXISTS(SELECT 1 FROM listings l where l.product_id = p.id and availability = true) as availability FROM products p ORDER BY p.time_added DESC LIMIT $1", [count]);
    return result.rows;
}
